...
class ContentQuerySet(models.query.QuerySet):
    ...
    def sliders(self):
        return self.filter(categories__contains='slider').filter(status=1).order_by('-updated')
...
class Content(models.Model):
    ...
    @property
    def author(self):
        email = self.added_by.email
        return "Admin" if email == 'mawardi@mac.com' else self.added_by.full_name

    @property
    def judul(self):
        bhs = get_language()
        return self.title if bhs == 'en' else self.title_id

    @property
    def artikel(self):
        bhs = get_language()
        return self.article if bhs == 'en' else self.article_id
